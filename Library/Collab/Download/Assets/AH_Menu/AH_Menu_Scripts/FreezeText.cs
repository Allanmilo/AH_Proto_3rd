...
public class FreezeText : MonoBehaviour
{
    ...
    Vector2 viewPos; // Variable for storing the current transform position.
   ...
   public bool bounceOn; //Switch to set pucks inside boundaries or to allow them to speed up and fly off screen'
    ...
    void Start()
    {
       ...
       // _menuPuckText.SetActive(true);
        ...
        bounceOn = true; // Starts bool as true.
            ...
            viewPos = transform.position; // Sets viewPos variable at start to current transform position.
            xPos = x; // Stores x variable value in variable xPos.
            yPos = y; // Stores y variable value in variable yPos.

         screenBounds = MainCamera.ScreenToWorldPoint(new Vector2(Screen.width, Screen.height));  // Sets the value of variable screenBound to screens width and height.
    ...
    {    
     ...
     private Vector2 RandomVector(float min, float max)  // Function to return random number vector used for velocity.
     {

...
namespace Unity.PlasticSCM.Editor.Views.PendingChanges
{
    internal static class DrawCommentTextArea
    {
            ...
            bool isOperationRunning)
        {
            using (new GuiEnabled(!isOperationRunning))
            {
                ...
                if (EditorGUI.EndChangeCheck())
                    OnTextAreaChanged(pendingChangesTab, pendingChangesTab.CommentText);
        ...
        static void OnTextAreaChanged(PendingChangesTab pendingChangesTab, string text)
        {
            pendingChangesTab.UpdateIsCommentWarningNeeded(text);
        }
